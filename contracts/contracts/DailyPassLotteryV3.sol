...
/**
 * @title DailyPassLotteryV3
 * @notice Fully on-chain daily lottery with DELEGATION SUPPORT and FIXED ACCOUNTING
 * @author EmpowerTours
 *
 * === V3 IMPROVEMENTS ===
 * - FIXED: Caller rewards now properly deducted from escrow amount
 * - Escrow amount = prize pool - caller rewards paid
 * - This prevents "MON failed" errors when winners try to claim
 *
 * === V2 IMPROVEMENTS ===
 ...
 */
contract DailyPassLotteryV3 is Ownable, ReentrancyGuard {
    ...
    struct DailyRound {
        ...
        address winner;
        uint256 winnerIndex;
        // V3: Track caller rewards paid for this round
    ...
    function commitRandomness(uint256 roundId) external nonReentrant {
        ...
        // V3: Pay caller reward and track it
        uint256 reward = 0;
        if (address(this).balance >= CALLER_REWARD) {
            ...
            if (!success) {
                reward = 0;
                round.callerRewardsPaid -= CALLER_REWARD; // Revert tracking if payment failed
            }
        }
    ...
    function revealWinner(uint256 roundId) external nonReentrant {
        ...
        // V3: Pay caller reward and track it BEFORE creating escrow
        uint256 reward = 0;
        if (address(this).balance >= CALLER_REWARD) {
            reward = CALLER_REWARD;
            round.callerRewardsPaid += reward;
        ...
        // V3: Create escrow with ADJUSTED amounts (prize pool - caller rewards)
        ...
        // V3: Auto-start next round after winner is revealed
        _checkAndRotateRound();
    }
        ...
        for (uint256 i = currentRoundId; i > minRound; i--) {
            ...
            if (round.status == RoundStatus.Active || round.status == RoundStatus.CommitPending) {
                if (block.timestamp >= round.endTime && round.commitBlock == 0) {
                    ...
                    // V3: No caller reward for lazy finalization
    ...
    function _claimPrize(address beneficiary, uint256 roundId) internal {
        ...
        // V3: Auto-start next round after winner claims (if current round is finalized)
        _checkAndRotateRound();
    }
    ...
    function reclaimExpiredEscrow(uint256 roundId) external onlyOwner nonReentrant {
        ...
        // V3: Auto-start next round after expired escrow is reclaimed
        _checkAndRotateRound();
    }
    ...
    function _startNewRound() internal {
        ...
        rounds[currentRoundId] = DailyRound({
            ...
            winner: address(0),
            winnerIndex: 0,
            callerRewardsPaid: 0 // V3: Initialize caller rewards tracking
        });
