 ...
 * @dev V2 adds burning functionality similar to EmpowerToursNFT
 */
contract ItineraryNFTv2 is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    constructor(address _passportContract, address _toursToken)
        ERC721("EmpowerTours Itinerary V2", "ITIN-V2")
        Ownable(msg.sender)
    {
    ...
    // ============================================
    // V2: Burning Functions
    // ============================================

    /**
     * @notice Owner burns their own itinerary NFT
     * @param tokenId The token ID to burn
     */
    function burnItinerary(uint256 tokenId) external nonReentrant {
        require(ownerOf(tokenId) == msg.sender, "Not token owner");

        // Mark experience as inactive
        experiences[tokenId].active = false;

        // Burn the NFT
        _burn(tokenId);

        emit ItineraryBurned(tokenId, msg.sender, msg.sender);
    }

    /**
     * @notice Burn itinerary on behalf of owner (requires approval)
     * @param owner The owner of the token
     * @param tokenId The token ID to burn
     */
    function burnItineraryFor(address owner, uint256 tokenId) external nonReentrant {
        require(ownerOf(tokenId) == owner, "Incorrect owner");

        // Check authorization - caller must be approved or operator
        require(
        ...
        emit ItineraryBurned(tokenId, msg.sender, owner);
    }

    /**
     * @notice Burn itinerary via delegated authorization (for Safe/smart wallet)
     * @dev Caller must be in authorizedBurners mapping
     * @param owner The owner of the token
     * @param tokenId The token ID to burn
     */
    function burnItineraryForDelegated(address owner, uint256 tokenId) external nonReentrant {
    ...
    /**
     * @notice Authorize/deauthorize an address to burn NFTs
