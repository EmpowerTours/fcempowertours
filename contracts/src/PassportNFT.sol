...
/**
 * @title PassportNFT
 * @notice EmpowerTours Passport - Streamlined for Monad Testnet
 *
 * === FEATURES ===
 ...
 */
contract PassportNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC721("EmpowerTours Passport", "ETPASS")
        Ownable(msg.sender)
    {
    ...
    function mint(
        ...
        return mintFor(msg.sender, userFid, countryCode, countryName, region, continent, uri);
    }
     ...
     * @dev Proof should be IPFS hash containing: GPS data, photo, attestation
     * @param tokenId Passport to verify
     * @param proofIPFSHash IPFS hash (e.g., "QmXXX...")
     */
     ...
     * @dev Called by oracle after validating proof via Gemini Maps API
     */
     ...
     * @dev Use Gemini Google Maps Grounding API to get placeId, coordinates
     */
    ...
    function addItineraryStamp(
        ...
        emit ItineraryStampAdded(tokenId, itineraryId, locationName, placeId, gpsVerified, block.timestamp);
    }
     ...
     *
     * Base score: 100
     * Venue stamps: +10 each (+5 bonus if verified)
     * Itinerary stamps: +15 each (+10 bonus if GPS verified)
     * Verified passport: 2x multiplier on ALL bonuses
     */
