...
/**
 * The address that is `msg.sender` for everything sent through {@link sendSafeTransaction}.
 *
 * Exported because callers need it to reason about the Safe's own state — its token allowances,
 * and whether a contract has authorised it. The passport mint needs both, and the absence of
 * either surfaces as an opaque revert rather than a message anyone can act on.
 */
export const safeAddress = SAFE_ACCOUNT;
...
export async function createSafeSmartAccountClient(): Promise<SmartAccountClient> {
  ...
  try {
    ...
    try {
      ...
      // A keccak256 digest, not a secret: keccak256("fallback_manager.handler.address"), the
      // storage slot Safe keeps its fallback handler in. Named here because the pre-commit
      // secret scanner cannot tell a 32-byte hash from a private key and asks for exactly this.
      ...
      console.warn(
        "   ⚠️ Could not read Safe modules (Safe may not be deployed)",
      );
    }
...
export async function sendSafeTransaction(
  ...
  try {
    ...
    if (!smartAccountClient.account) {
      throw new Error(
        "Failed to create smart account client - account is undefined",
      );
    }
    ...
    console.log(
      "✅ EntryPoint is deployed (code length: " + entryPointCode.length + ")",
    );
    ...
    if (!accountCode || accountCode === "0x") {
      throw new Error(
        `Safe account ${SAFE_ACCOUNT} is NOT deployed! Deploy it first before using AA.`,
      );
    }
    console.log(
      "✅ Safe account is deployed (code length: " + accountCode.length + ")",
    );
    ...
    if (safeBalance < ABSOLUTE_MINIMUM_BALANCE) {
      ...
      const deficitMON = Math.max(
        0,
        parseFloat(requiredMON) - parseFloat(currentBalanceMON),
      ).toFixed(4);
    ...
    if (safeBalance < RECOMMENDED_BALANCE) {
      ...
      const deficitMON = Math.max(
        0,
        parseFloat(recommendedMON) - parseFloat(currentBalanceMON),
      ).toFixed(4);
      ...
      console.warn(
        `   This transaction may fail with "reserve balance check" errors.`,
      );
      console.warn(`   Please consider funding the Safe wallet soon.`);
    } else if (safeBalance < OPTIMAL_BALANCE) {
      console.log(
        `ℹ️  Safe balance is adequate but could be higher for 24/7 operations`,
      );
    ...
    for (let i = 0; i < calls.length; i++) {
      ...
      if (functionSelector === "0x095ea7b3") {
        ...
        if (isMaxApproval) {
          console.log(
            `   [Call ${i}] ℹ️  Max approval detected - skipping balance check`,
          );
          console.log(
            `   [Call ${i}] Note: Actual spend amount will be validated in the next call`,
          );
        } else {
          ...
          try {
            ...
            console.log(
              `   [Call ${i}] Safe's token balance: ${balance.toString()}`,
            );

            if (balance < amount) {
              console.warn(
                `   [Call ${i}] ⚠️  Approving ${amount} but Safe only has ${balance}`,
              );
              console.warn(
                `   [Call ${i}] This is OK for approve operations, but may fail on actual spend`,
              );
            } else {
              console.log(
                `   [Call ${i}] ✅ Safe has sufficient balance for approval amount`,
              );
            }
          } catch (balanceErr: any) {
            console.error(
              `   [Call ${i}] ⚠️  Balance check failed:`,
              balanceErr.message,
            );
          }
        }
        ...
        try {
          ...
          const isAccepted = await publicClient.readContract({
            ...
            abi: parseAbi([
              "function acceptedNFTs(address) external view returns (bool)",
            ]),
            functionName: "acceptedNFTs",
            args: [nftAddress as Address],
          });
          console.log(
            `   [Call ${i}] NFT whitelisted in YieldStrategy: ${isAccepted}`,
          );

          if (!isAccepted) {
            throw new Error(
              `NFT address ${nftAddress} is not whitelisted in YieldStrategy. The NFT contract must be added to the whitelist before staking. Please contact support.`,
            );
          }
          console.log(`   [Call ${i}] ✅ NFT is whitelisted`);
        } catch (whitelistErr: any) {
          console.error(
            `   [Call ${i}] ❌ NFT whitelist check failed:`,
            whitelistErr.message,
          );
          throw new Error(
            `NFT whitelist validation failed: ${whitelistErr.message}`,
          );
        }
        ...
        try {
          ...
          if (nftOwner.toLowerCase() !== beneficiary.toLowerCase()) {
            throw new Error(
              `Beneficiary ${beneficiary} does not own NFT #${nftTokenId} (owned by ${nftOwner})`,
            );
          }
          console.log(`   [Call ${i}] ✅ Beneficiary owns the NFT`);
        } catch (ownerErr: any) {
          console.error(
            `   [Call ${i}] ❌ NFT ownership check failed:`,
            ownerErr.message,
          );
          throw new Error(
            `NFT ownership validation failed: ${ownerErr.message}`,
          );
        }
        ...
        try {
          ...
          const isUsed = await publicClient.readContract({
            address: yieldStrategyAddress,
            abi: parseAbi([
              "function nftCollateralUsed(address, uint256) external view returns (bool)",
            ]),
            functionName: "nftCollateralUsed",
            args: [nftAddress as Address, nftTokenId],
          });
          console.log(
            `   [Call ${i}] NFT already used as collateral: ${isUsed}`,
          );

          if (isUsed) {
            throw new Error(
              `NFT #${nftTokenId} is already being used as collateral in another staking position. You cannot stake with the same NFT twice.`,
            );
          }
          console.log(`   [Call ${i}] ✅ NFT is available for staking`);
        } catch (collateralErr: any) {
          console.error(
            `   [Call ${i}] ❌ NFT collateral check failed:`,
            collateralErr.message,
          );
          throw new Error(
            `NFT collateral validation failed: ${collateralErr.message}`,
          );
        }
        ...
        try {
          const yieldStrategyAddress = call.to;
          const isSafeBeneficiary =
            beneficiary.toLowerCase() === SAFE_ACCOUNT.toLowerCase();

          if (!isSafeBeneficiary) {
            console.log(
              `   [Call ${i}] ⚠️  Skipping NFT approval check - beneficiary (${beneficiary}) is not the Safe`,
            );
            console.log(
              `   [Call ${i}] Note: In delegated staking, the beneficiary must have pre-approved the YieldStrategy`,
            );
            console.log(
              `   [Call ${i}] The contract will validate approval during execution`,
            );
          } else {
            ...
            console.log(
              `   [Call ${i}] NFT approved address: ${approvedAddress}`,
            );
            ...
            console.log(
              `   [Call ${i}] NFT approved for all: ${isApprovedForAll}`,
            );

            const hasApproval =
              approvedAddress.toLowerCase() ===
                yieldStrategyAddress.toLowerCase() || isApprovedForAll;
          ...
          console.error(
            `   [Call ${i}] ❌ NFT approval check failed:`,
            approvalErr.message,
          );
          throw approvalErr;
        }
    ...
    if (calls.length > 1) {
      console.log(
        "✅ Batch transaction detected - skipping individual call simulation",
      );
      console.log(
        "   Batched calls will be validated as a whole during execution",
      );
    } else {
      ...
      try {
        ...
        if (functionSelector === "0xb438aa31") {
          ...
          try {
            ...
            if (
              errMsg.includes("Invalid NFT address") ||
              errMsg.includes("acceptedNFTs")
            ) {
              throw new Error(
              ...
              errMsg.includes("Beneficiary must own NFT") ||
              errMsg.includes("ownerOf")
            ) {
              throw new Error(
              ...
              errMsg.includes("NFT already used") ||
              errMsg.includes("collateral")
            ) {
              throw new Error(
    ...
    for (let i = 0; i < calls.length - 1; i++) {
      ...
      if (currentSelector === "0x095ea7b3") {
        ...
        if (approveSpender.toLowerCase() === nextTarget.toLowerCase()) {
          ...
          console.log(
            `⚠️  Detected approve + spend pattern (Call ${i} -> Call ${i + 1})`,
          );
          console.log(
            "   This pattern will fail bundler gas estimation but succeed in execution",
          );
          break;
        }
      }
    ...
    if (hasApproveSpendPattern) {
      console.log(
        "🔧 Using fixed gas values for approve + spend pattern (skipping estimation)",
      );
      console.log(
        "   Bundler gas estimation will fail for this pattern, but execution will succeed",
      );
      ...
      console.log(
        "   Using fixed gas values:",
        JSON.stringify(estimatedGas, (_, v) =>
      ...
      try {
        console.log(
          "🔍 Attempting manual gas estimation with initial limits...",
        );
        console.log(
          "   Initial callGasLimit:",
          initialGasLimits.callGasLimit.toString(),
        );
        console.log(
          "   Initial verificationGasLimit:",
          initialGasLimits.verificationGasLimit.toString(),
        );
        console.log(
          "   Initial preVerificationGas:",
          initialGasLimits.preVerificationGas.toString(),
        );
        ...
        if (
          errorMsg.includes("reverted during simulation with reason: 0x") ||
          errorMsg.includes("reason: 0x")
        ) {
          console.error(
            "   ⚠️ EMPTY REVERT DETECTED: The UserOperation simulation failed with no revert reason.",
          );
          console.error("   This usually indicates one of the following:");
          console.error(
            "   1. The Safe account validation logic is rejecting the operation",
          );
          console.error(
            "   2. One of the calls in the batch is reverting without a message",
          );
          console.error(
            "   3. The EntryPoint detected an invalid operation state",
          );
          console.error(
            "   4. Insufficient gas for the operation (try with higher limits)",
          );
          console.error(
        ...
        try {
          console.log(
            "🔍 Retry with maxGasLimits:",
            JSON.stringify(maxGasLimits, (_, v) =>
              typeof v === "bigint" ? v.toString() : v,
            ),
          );
          ...
          console.log(
            "✅ Gas estimation succeeded with higher limits:",
            JSON.stringify(estimatedGas, (_, v) =>
              typeof v === "bigint" ? v.toString() : v,
            ),
          );
          ...
          console.error(
            "❌ Gas estimation failed even with maximum limits:",
            redact({
          ...
          if (
            errorDetails.includes("insufficient") ||
            errorDetails.includes("balance")
          ) {
            errorDetails =
              "Insufficient token balance. Please ensure the Safe has enough tokens for this operation.";
          } else if (
            errorDetails.includes("not owner") ||
            errorDetails.includes("ownership")
          ) {
            errorDetails =
              "NFT ownership verification failed. Please ensure you own the required NFT.";
          } else if (
            errorDetails.includes("revert") &&
            errorDetails.includes("0x")
          ) {
            errorDetails =
              "Transaction would revert. This could be due to: 1) Insufficient token balance, 2) Missing NFT ownership, 3) Invalid contract state, or 4) Missing approvals.";
          }
    ...
    const gasWithBuffer = {
      ...
      verificationGasLimit:
        (estimatedGas.verificationGasLimit * bufferPercent) / 100n,
      preVerificationGas:
        (estimatedGas.preVerificationGas * bufferPercent) / 100n,
    };

    console.log(
      `🚀 Using gas estimates with ${bufferLabel} buffer:${hasApproveSpendPattern ? " (approve+spend pattern detected)" : ""}`,
    );
    console.log("   callGasLimit:", gasWithBuffer.callGasLimit.toString());
    console.log(
      "   verificationGasLimit:",
      gasWithBuffer.verificationGasLimit.toString(),
    );
    console.log(
      "   preVerificationGas:",
      gasWithBuffer.preVerificationGas.toString(),
    );
    ...
    console.log(
      "   Track your UserOperation: https://monadscan.com/op/" + userOpHash,
    );

    // Wait for the UserOperation to be included in a transaction
    console.log(
      "⏳ Waiting for UserOperation to be mined (timeout: 5 minutes)...",
    );
    ...
    try {
      ...
      const userOpEventTopic =
        "0x49628fd1471006c1482da88028e9ce4dbb080b815c9b0344d39e5a8e6ec1419f"; // keccak256("UserOperationEvent(bytes32,address,address,uint256,bool,uint256,uint256)")

      const userOpEvent = receipt.receipt.logs.find(
        (log: any) =>
          log.topics[0]?.toLowerCase() === userOpEventTopic.toLowerCase(),
      );

      if (!userOpEvent) {
        console.warn("⚠️  Could not find UserOperationEvent in receipt logs");
        console.warn(
          "   Proceeding with caution - unable to verify internal call success",
        );
      } else {
        ...
        if (!success) {
          console.error(
            "❌ UserOperation FAILED - ERC-4337 silent failure detected!",
          );
          console.error("   Transaction hash:", txHash);
          console.error("   UserOp hash:", userOpHash);
          console.error(
            "   The blockchain transaction succeeded (status=1) but the internal calls failed (success=0)",
          );
        ...
        console.log(
          "✅ UserOperation success validated - internal calls executed successfully",
        );
      }
      ...
      try {
        ...
        if (receiptData.result) {
          ...
          if (success === false) {
            console.error(
              "❌ UserOperation FAILED (manual check) - ERC-4337 silent failure detected!",
            );
            throw new Error(
              `UserOperation failed: Internal calls did not execute successfully. ` +
                `Transaction ${txHash} succeeded on-chain but did not perform the intended action. ` +
    ...
    if (
      errorStr.includes("Insufficient Pimlico balance") ||
      errorStr.includes("pm_getPaymasterData")
    ) {
      console.warn(
        "⚠️  Pimlico paymaster has insufficient balance. Falling back to direct Safe execution...",
      );
      console.warn(
        "   This bypasses ERC-4337 and executes through the Safe contract directly.",
      );
      try {
        ...
        console.error(
          "❌ Direct Safe execution also failed:",
          directErr.message,
        );
        throw new Error(
    ...
    if (
      error.message?.includes("reserve balance check") ||
      error.details?.includes("reserve balance check")
    ) {
      const currentBalance = (
        Number(await publicClient.getBalance({ address: SAFE_ACCOUNT })) / 1e18
      ).toFixed(4);
      ...
      console.error(
        "   Gas prices may increase requirements during high network activity",
      );
...
export async function checkSafeBalance(
  requiredAmount: bigint,
): Promise<boolean> {
  const balance = await getSafeBalance();
  const hasBalance = balance >= requiredAmount;
  console.log(
    `✅ Balance check: ${balance.toString()} >= ${requiredAmount.toString()} = ${hasBalance}`,
  );
  return hasBalance;
}
...
export async function sendSafeTransactionDirect(
  ...
  console.log(
    "📤 [DirectSafe] Executing transaction directly (bypassing Pimlico AA)...",
  );
  ...
  if (!SAFE_OWNER_PRIVATE_KEY) {
    throw new Error(
      "SAFE_OWNER_PRIVATE_KEY not set - cannot execute direct Safe transaction",
    );
  }
  ...
  console.log(
    "💰 [DirectSafe] Owner EOA balance:",
    (Number(ownerBalance) / 1e18).toFixed(4),
    "MON",
  );
  ...
  if (calls.length === 1) {
    ...
    const MULTISEND_ADDRESS =
      "0x38869bf66a61cF6bDB996A6aE40D5853Fd43B526" as Address;
    ...
    const paddedData =
      dataBytes + "0".repeat((64 - (dataBytes.length % 64)) % 64);

    execTo = MULTISEND_ADDRESS;
    execData =
      `${multiSendSelector}${offsetHex}${dataLenHex}${paddedData}` as Hex;
    execValue = 0n;
  ...
  const sig = await sign({
    hash: safeTxHash,
    privateKey: SAFE_OWNER_PRIVATE_KEY,
  });
  ...
  const packedSignature =
    `${sig.r}${sig.s.slice(2)}${Number(v).toString(16).padStart(2, "0")}` as Hex;
  ...
  if (receipt.status === "reverted") {
    throw new Error(
      `[DirectSafe] execTransaction reverted (tx: ${txHash}). Check Safe owner signature and call data.`,
    );
  }

  console.log(
    "✅ [DirectSafe] Transaction confirmed in block:",
