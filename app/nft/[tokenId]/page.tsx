...
async function resolveFidFromWallet(
  ...
  try {
    ...
    // Proxied through /api/neynar so the API key stays server-side.
    const url = `/api/neynar/v2/farcaster/user/bulk_by_address?addresses=${walletAddress}`;
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-xl">
            {isArtHint ? "Loading art..." : "Loading music..."}
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-500 text-sm">
            Try again in a few moments if this is a newly minted token
          </p>
          <button
      ...
      <div className="border-b border-purple-500/20 bg-slate-900/50 backdrop-blur sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-6 py-4">
          ...
          <p className="text-gray-400 text-xs mt-1">
            {isArt ? "Art NFTs on Monad" : "Stream Music NFTs on Monad"}
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          {collectorData?.collectorImageUrl || imageUrl ? (
            <div className="relative">
              <div
                className={`w-full max-w-xs aspect-square rounded-3xl overflow-hidden shadow-2xl ${collectorData?.isCollectorMaster ? "border-4 border-amber-400/60" : "border-4 border-purple-500/30"} flex items-center justify-center bg-gray-900`}
              >
                <img
        ...
        <div className="bg-gradient-to-r from-purple-600/20 to-blue-600/20 border border-purple-500/30 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <p className="text-gray-400 text-sm mb-1">
              {isArt ? "ARTWORK TITLE" : "SONG TITLE"}
            </p>
          ...
          <div>
            ...
            <p className="text-xl text-gray-300 font-mono break-all">
              {nftData.artist}
            </p>
          </div>
            ...
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-bold">
                  {collectorData.collectorsMinted} / {collectorData.maxEditions}
                </span>
              </div>
              {parseFloat(collectorData.collectorPrice) > 0 && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-amber-400 font-bold">
                    {collectorData.collectorPrice} WMON
                  </span>
                </div>
              )}
            </div>
