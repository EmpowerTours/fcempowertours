...
export default function DynamicCastFrame() {
  ...
  useEffect(() => {
    const fetchCasts = async () => {
      try {
        const params = activeCategories.length > 0 
        ...
        setCasts(prevCasts => {
          const newCasts = data.casts.filter((cast: Cast) => 
            !prevCasts.some(existing => existing.id === cast.id)
          );
  ...
  const handleTouchEnd = useCallback(async (e: React.TouchEvent) => {
    ...
    if (gestureStart.x > screenWidth * 0.7 && 
    ...
    if (gestureStart.x < screenWidth * 0.3 && 
        gestureStart.y > screenHeight * 0.7 &&
        deltaX > screenWidth * 0.5 && 
        deltaY < -screenHeight * 0.5) {
  ...
  const startVideoRecording = async () => {
    // Video recording implementation (similar to previous)
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        {/* Right edge */}
        <div className="absolute top-0 right-0 bottom-0 w-16 overflow-hidden">
          <motion.div 
          ...
          >
            {casts.map((cast, i) => (
              <div key={`right-${i}`} className="text-purple-400 text-sm my-4 opacity-80 transform rotate-90 origin-center">
        ...
        {/* Left edge */}
        <div className="absolute top-0 left-0 bottom-0 w-16 overflow-hidden">
          <motion.div 
            className="flex flex-col"
            animate={{ y: [1000, -1000] }}
            transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
          >
            {casts.map((cast, i) => (
              <div key={`left-${i}`} className="text-purple-400 text-sm my-4 opacity-80 transform -rotate-90 origin-center">
        ...
        <div className="absolute top-20 left-20 bg-black/50 backdrop-blur-sm rounded-lg p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {activeCategories.map(cat => (
              <span key={cat} className="px-3 py-1 bg-purple-600 text-white rounded-full text-xs">
                {cat}
              </span>
            ))}
          </div>
