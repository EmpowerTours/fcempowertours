...
export function LiveRadioModal({
  onClose,
  isDarkMode = true,
  minimized: externalMinimized,
  setMinimized: externalSetMinimized,
  onAudioPlay,
  registerPauseAudio,
}: LiveRadioModalProps) {
  ...
  const [voiceNoteType, setVoiceNoteType] = useState<"shoutout" | "ad" | null>(
    null,
  );
  const [recordingStatus, setRecordingStatus] = useState<
    "idle" | "recording" | "uploading" | "recorded"
  >("idle");
  ...
  const [leaderboard, setLeaderboard] = useState<
    { address: string; totalSongsListened: number; currentStreak: number }[]
  >([]);
  ...
  const resolveArtistNames = useCallback(
    async (fids: number[]) => {
  ...
  const fetchListenerStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/live-radio?action=listener-stats&address=${walletAddress}`,
      );
      const data = await response.json();
  ...
  const fetchRecentPlays = useCallback(async () => {
    try {
      const response = await fetch(
        "/api/live-radio?action=play-history&limit=10",
      );
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    const handleEnded = async () => {
      console.log(
        "[LiveRadio] Audio ended, isPlayingVoiceNote:",
        isPlayingVoiceNoteRef.current,
      );
      ...
      if (isPlayingVoiceNoteRef.current) {
        ...
        if (radioState?.currentSong && audioRef.current) {
          console.log(
            "[LiveRadio] Switching to song after voice note:",
            radioState.currentSong.name,
          );
        ...
        if (radioState?.currentSong && walletAddress) {
          try {
            ...
            console.log(
              "[LiveRadio] Reported song end to server:",
              radioState.currentSong.name,
            );
          } catch (error) {
  ...
  useEffect(() => {
      ...
      if (lastVoiceNoteIdRef.current !== voiceNoteId) {
        ...
        if (isPlaying) {
          ...
          audioRef.current
            ...
            .catch((e) =>
              console.warn("[LiveRadio] Voice note autoplay blocked:", e),
            );
        } else {
          console.log(
            "[LiveRadio] 🎤 Voice note loaded but isPlaying=false, waiting for user to play",
          );
        }
      }
    ...
    if (isPlayingVoiceNoteRef.current) {
      console.log(
        "[LiveRadio] Server cleared voice note, but client still playing - waiting for audio to end",
      );
      return;
    }
    ...
    if (lastSongIdRef.current !== currentSongId) {
      ...
      if (isPlaying) {
        ...
        audioRef.current
          .play()
          .catch((e) => console.warn("[LiveRadio] Song autoplay blocked:", e));
      }
    }
  ...
  const handleQueueSong = async (song: any) => {
    ...
    try {
      ...
      console.log(
        "[LiveRadio] Queueing song on-chain:",
        song.name,
        "tokenId:",
        song.tokenId,
      );
      ...
      if (data.success) {
        ...
        showToast(
          `Now playing: "${song.name}"! TX: ${txHash.slice(0, 10)}...`,
          "success",
        );
      } else {
  ...
  const submitVoiceNote = async () => {
    ...
    try {
      ...
      formData.append(
        "file",
        recordedAudioBlob,
        `voice-${voiceNoteType}-${Date.now()}.webm`,
      );
  ...
  const handleSkipToRandom = async () => {
    ...
    try {
      ...
      showToast(
        "Requesting random song... Pyth Entropy processing!",
        "success",
      );
    ...
    >
            ...
            <>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {radioState?.isLive && (
                    <span className="text-xs text-gray-400">
                      • {radioState.listenerCount || 0} listeners
                    </span>
                  )}
                  {connectionStatus === "connected" && (
                    <span className="text-xs text-green-500 opacity-75">
                      • SSE
                    </span>
                  )}
                ...
                {radioState?.currentSong ? (
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      >
                        {getArtistDisplayName(
                          ...
                          availableSongs.find(
                            (s) =>
                              s.tokenId === radioState.currentSong?.tokenId,
                          )?.artistFid,
                        )}
                      </p>
                    ...
                    <div className="mt-4">
                      <div
                        className={`flex items-center justify-between text-xs mb-1 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                      >
                        <span>
                          {formatTime(
                            Math.floor(
                  ...
                  >
                    ...
                    >
                      Queue a song to start the party!
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                ...
                >
                  ...
                  <p
                    className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {pricing.voiceNote} WMON (3-5 sec)
                  </p>
                </button>
                <button
                  onClick={handleSkipToRandom}
              ...
              <div className="mb-6">
                ...
                {queue.length > 0 ? (
                  ...
                  <p
                    className={`text-center text-sm py-4 ${isDarkMode ? "text-gray-500" : "text-gray-500"}`}
                  >
                    Queue is empty
                  </p>
                )}
      ...
      {showQueueModal && (
        ...
        >
          ...
          >
            <h3
              className={`text-base sm:text-lg font-bold mb-3 ${isDarkMode ? "text-white" : "text-gray-900"}`}
            >
              Queue a Song
            </h3>
            <p
              className={`text-xs sm:text-sm mb-3 break-words ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
            >
              ...
              {loadingSongs ? (
                ...
                availableSongs.map((song) => (
                  ...
                  >
                    ...
                    <p
                      className={`text-xs sm:text-sm font-medium truncate ${isDarkMode ? "text-white" : "text-gray-900"}`}
                    >
                      {song.name || `Song #${song.tokenId}`}
                    </p>
                    <p
                      className={`text-xs truncate ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                    >
                      {getArtistDisplayName(song.artist, song.artistFid)}
                    </p>
                  </button>
                ))
              ) : (
            ...
            {selectedSong && (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span
                    className={`text-xs font-semibold ${isDarkMode ? "text-orange-400" : "text-orange-700"}`}
                  >
                    WMON
                  </span>
                </div>
            ...
            {/* Payment Info */}
            <div
              className={`mb-3 p-2 rounded-lg ${isDarkMode ? "bg-yellow-500/10 border border-yellow-500/30" : "bg-yellow-50 border border-yellow-200"}`}
            >
              <p
                className={`text-xs text-center ${isDarkMode ? "text-yellow-400" : "text-yellow-700"}`}
              >
                Payment: {pricing.queueSong} WMON
                {tipAmount && parseFloat(tipAmount) > 0
                  ? ` + ${tipAmount} WMON tip`
                  : ""}{" "}
                via delegated transaction
              </p>
            </div>
