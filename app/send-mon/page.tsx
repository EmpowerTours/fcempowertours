...
export default function SendMonPage() {
  ...
  const handleSend = async () => {
    ...
    const senderAddress = process.env.NEXT_PUBLIC_PLATFORM_SAFE || "";
    ...
    try {
      ...
      try {
        const username =
          user?.username || fid?.toString() || senderAddress.slice(0, 8);
          ...
          <div className="text-center">
            ...
            <div className="bg-red-500/20 border border-red-500 rounded-lg p-4">
              <p className="text-red-200 text-sm">
                Failed to load Farcaster SDK: {farcasterError.message}
              </p>
            </div>
          </div>
        </div>
  ...
  const safeWalletAddress = process.env.NEXT_PUBLIC_PLATFORM_SAFE || "";
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 max-w-md w-full space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-300 text-sm">
            Send MON from your Farcaster Safe wallet
          </p>
        </div>
        ...
        <>
          <div className="bg-yellow-500/20 border border-yellow-500 rounded-xl p-4 mb-4">
            ...
            <p className="text-yellow-100 text-xs">
              Monad is not in Farcaster's token list yet. To send MON to your
              Safe:
            </p>
          ...
          <div className="bg-black/20 rounded-xl p-4">
            ...
            {fid && <p className="text-blue-400 text-xs mt-2">💡 FID: {fid}</p>}
          </div>
          ...
          {success && (
            <div className="bg-green-500/20 border border-green-500 rounded-lg p-4 space-y-2">
              ...
              {txHash && (
                <>
                  <p className="text-green-200 text-xs break-all font-mono">
                    {txHash}
                  </p>
                  <a
