...
export async function GET(req: NextRequest) {
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error("Safe info fetch timed out after 30s")),
        30000,
      ),
    );
    ...
    const isAdequatelyFunded =
      parseFloat(safeInfo.balance) >= RECOMMENDED_SAFE_BALANCE;
