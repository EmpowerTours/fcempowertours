...
async function getCountryFromIP(
  request: NextRequest,
): Promise<{ country: string; ip: string } | null> {
  try {
    // Get IP from headers
    ...
    console.log("🌍 Server-side geo verification:", {
      ip: data.ip,
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("🎫 Passport mint request:", {
      fid,
      countryCode,
      countryName,
      userAddress,
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Server-side geo verification failed. Make sure you're not using a VPN.",
        },
        { status: 403 },
      );
    }
    ...
    if (geoData.country !== countryCode && !isHongKongOverride) {
      console.warn(
        `🚨 GEO MISMATCH: User IP is in ${geoData.country} but requested ${countryCode}`,
      );
      return NextResponse.json(
        {
    ...
    if (!isValidCountryCode(countryCode)) {
      ...
      throw new Error(
        `Invalid country code: ${countryCode}. Must be one of 195 recognized countries.`,
      );
    }
    ...
    console.log(
      `🌍 Country validated: ${countryInfo.flag} ${countryInfo.name} (${countryInfo.region}, ${countryInfo.continent})`,
    );
    ...
    if (fid) {
      try {
        ...
        console.warn(
          "⚠️ Neynar lookup failed, proceeding anyway:",
          neynarError.message,
        );
      }
    }
    ...
    console.log(
      `🔍 Checking for existing ${countryCode} passport for ${recipientAddress}...`,
    );
    ...
    try {
      ...
      const existingPassports = await findAllPassports(
      ...
      if (existingPassports.length > 0) {
        console.log(
          `❌ DUPLICATE DETECTED: User already has ${countryCode} passport #${existingPassports[0].tokenId}`,
        );
        await redis.del(lockKey); // Release lock
      ...
      console.log(
        `✅ No existing ${countryCode} passport found - proceeding with mint`,
      );
    } catch (checkError) {
      // 🛡️ ANTI-GAMING: Fail-closed - block mints on any verification error
      console.error(
        "❌ Passport verification error - blocking mint:",
        checkError,
      );
      await redis.del(lockKey); // Release lock
      return NextResponse.json(
        ...
        return NextResponse.json(
          {
            ...
            cooldownReason:
              "24-hour cooldown between passports (anti-gaming measure)",
    ...
    const finalMetadata = generatePassportMetadata(
      countryCode,
      countryName,
      tokenId,
    );
    const finalTokenURI = await uploadMetadataToPinata(
      finalMetadata,
      countryCode,
      tokenId,
    );
...
@empowertours`;
        ...
        console.warn(
          "⚠️ Cast failed (mint still succeeded):",
          castError.message,
        );
