...
function verifyWebhookSignature(
  ...
  if (!WEBHOOK_SECRET) {
    console.error(
      "[Webhook] NEYNAR_WEBHOOK_SECRET not configured - REJECTING request",
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const text = (cast.text || "").toLowerCase().slice(0, 500); // Limit command length
    ...
    switch (command.type) {
      case "mint_music":
        ...
        break;
      case "mint_passport":
        txHash = await mintNFT("passport", authorFid, {
          countryCode: "US",
      ...
      default:
        await replyCast(
          cast.hash,
          'Unknown command. Try: "mint music", "mint passport", "buy itinerary 1"',
        );
        return NextResponse.json({ ok: true });
    }
    ...
    if (txHash) {
      await replyCast(
        cast.hash,
        `@${cast.author.username} Executed ${command.type}! Tx: ${txHash}\n${EXPLORER_URL}/tx/${txHash}`,
      );
    }
    return NextResponse.json({ ok: true });
  } catch (err: any) {
