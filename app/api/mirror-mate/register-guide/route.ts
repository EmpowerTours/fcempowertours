...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[MirrorMate] Registering guide:", {
      fid,
      username,
      displayName,
      walletAddress,
    ...
    if (tokenId === 0n) {
      ...
      return NextResponse.json(
        {
          error:
            "You need a Passport NFT to register as a guide. Mint one at /passport first!",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Could not verify your Passport credit score. Please try again.",
        },
        { status: 400 },
      );
    }

    console.log("[MirrorMate] Passport found:", {
      tokenId: tokenId.toString(),
    ...
    if (USE_USER_SAFES) {
      ...
      const userSafeAddress = await getUserSafeAddress(
      ...
      const result = await sendUserSafeTransaction(
        walletAddress as Address,
        calls,
      );
      tx = result.txHash;
