...
export async function POST(request: NextRequest) {
  try {
    // SECURITY: rate limit — this pins files to paid Pinata storage.
    ...
    const rightsDeclarationRaw = formData.get("rightsDeclaration") as
      | string
      | null;
    ...
    if ((previewFile && !fullFile) || (!previewFile && fullFile)) {
      return NextResponse.json(
        {
          error:
            "If uploading music, please provide both previewAudio and fullAudio",
        },
        { status: 400 },
      );
    }
    ...
    if (!(coverFile instanceof File)) {
      return NextResponse.json(
        {
          error:
            "Invalid cover file upload. Cover must be a valid File object.",
        },
        { status: 400 },
      );
    }
    ...
    const uploadFileToPinata = async (
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        const timeoutId = setTimeout(() => {
          console.log(
            `⏱️ Timeout triggered for ${fileType} attempt ${attempt}`,
          );
          controller.abort();
        ...
        try {
          ...
          console.log(
            `📤 [Attempt ${attempt}/${maxRetries}] Uploading ${fileType} (${fileSizeKB}KB / ${fileSizeMB}MB)...`,
          );
          ...
          const isTimeout =
            errorCode === "ECONNABORTED" || error.name === "AbortError";
          ...
          if (attempt === maxRetries) {
            ...
            if (isTimeout) {
              throw new Error(
                `Upload timeout for ${fileType}: File may be too large. Try a smaller file or compress it.`,
              );
            } else if (isNetworkError) {
              throw new Error(
                `Network error uploading ${fileType}: Connection was reset. Please try again.`,
              );
            }
            throw new Error(
              `Failed to upload ${fileType} after ${maxRetries} attempts: ${errorMsg}`,
            );
          }
...
The result should look like a premium physical collector's vinyl or art print — luxurious but not overwhelming the original art. Keep the same composition and subject matter.`;
        ...
        if (
          ...
          imageResult.candidates[0]?.content?.parts
        ) {
        ...
        if (collectorImageBase64) {
          ...
          const collectorFile = new File(
            [blob],
            `collector-${(collectorTitle || description).replace(/\s+/g, "-").toLowerCase()}-${Date.now()}.png`,
            { type: "image/png" },
          );
          collectorCoverCid = await uploadFileToPinata(
            collectorFile,
            "collector cover art",
          );
          ...
          const collectorMetaRes = await axios.post(
            PINATA_JSON_URL,
            collectorMetadata,
            {
              headers: {
          ...
          if (collectorMetaRes.data.IpfsHash) {
            ...
            console.log(
              "👑 Collector metadata uploaded:",
              collectorMetadataCid,
            );
          }
        } else {
          console.warn(
            "⚠️ Gemini did not return an image, collector art will use standard cover",
          );
        }
      } catch (geminiError: any) {
        console.error(
          "⚠️ Gemini collector art generation failed, using standard cover:",
          geminiError.message,
        );
    ...
    if (isCollectorEdition && isArtOnly && !collectorMetadataCid) {
      console.log(
        "🖼️ Art collector edition — using original cover art for collector metadata",
      );
      ...
      const artCollectorMetaRes = await axios.post(
        PINATA_JSON_URL,
        collectorMetadata,
        {
          headers: {
      ...
      if (artCollectorMetaRes.data.IpfsHash) {
        ...
        console.log(
          "🖼️ Art collector metadata uploaded:",
          collectorMetadataCid,
        );
      }
    }
    ...
    if (rightsDeclarationRaw) {
      try {
        ...
        // falls back to v1.0 for it — so the payload below must record 1.0 too, or the stored
        ...
        if (agreementRes.data.IpfsHash) {
          ...
          console.log(
            "📜 Rights agreement uploaded to IPFS:",
            rightsAgreementCid,
          );
        ...
        console.error(
          "⚠️ Rights declaration processing failed (non-fatal):",
          rightsError.message,
        );
