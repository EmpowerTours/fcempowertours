...
import {
  ...
  DEFAULT_HOSPITALITY_RIDER,
} from "@/lib/epk/defaults";
...
const PLATFORM_SAFE = (process.env.NEXT_PUBLIC_SAFE_ACCOUNT ||
  process.env.NEXT_PUBLIC_PLATFORM_SAFE) as Address;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!WMON_ADDRESS || !PLATFORM_SAFE) {
      console.error(
        "[EPK Generate] WMON_ADDRESS or PLATFORM_SAFE not configured",
      );
      return NextResponse.json(
    ...
    const [farcasterProfile, streamingStats, genreData] = await Promise.all([
      ...
      getArtistStreamingStats(userAddress),
    ...
    return NextResponse.json(
      { error: error.message || "EPK generation failed" },
      { status: 500 },
    );
  }
}
...
async function generateEPKWithGemini(
  ...
  stats: Awaited<ReturnType<typeof getArtistStreamingStats>>,
  genres: string[],
  userAddress: string,
  userFid?: number,
): Promise<EPKMetadata> {
  ...
  try {
    ...
    const topSongTitles = stats.topSongs
      .slice(0, 5)
      .map((s) => `"${s.title}" (${s.plays} plays)`)
      .join(", ");
...
Be professional and concise. Do not fabricate achievements not in the data.`;
    ...
    const config = {
      ...
      responseSchema: {
        ...
        properties: {
          bio: {
            type: Type.STRING,
            description: "Professional artist bio, 150-250 words",
          },
          genre: {
            type: Type.ARRAY,
          ...
          location: {
            type: Type.STRING,
            description: 'Artist location or "Web3 Native / Global"',
          },
          technicalRider: {
            type: Type.OBJECT,
            properties: {
    ...
    const draft: EPKMetadata = {
      ...
      booking: {
        ...
        inquiryEnabled: true,
        availableFor: generated.booking?.availableFor || [
          "Crypto Conferences",
      ...
      technicalRider: {
        stage: {
          ...
          items:
            generated.technicalRider?.stage ||
            DEFAULT_TECHNICAL_RIDER.stage.items,
        },
        sound: {
          title: "Sound System",
          items:
            generated.technicalRider?.sound ||
            DEFAULT_TECHNICAL_RIDER.sound.items,
        },
        lighting: {
          title: "Lighting",
          items:
            generated.technicalRider?.lighting ||
            DEFAULT_TECHNICAL_RIDER.lighting.items,
        },
        video: {
          title: "Video / LED",
          items: generated.technicalRider?.video || [],
        },
        backline: {
          title: "Backline",
          items:
            generated.technicalRider?.backline ||
            DEFAULT_TECHNICAL_RIDER.backline.items,
        },
        soundcheck: {
          title: "Soundcheck",
          items:
            generated.technicalRider?.soundcheck ||
            DEFAULT_TECHNICAL_RIDER.soundcheck.items,
        },
        crew: {
          title: "Crew Requirements",
          items:
            generated.technicalRider?.crew ||
            DEFAULT_TECHNICAL_RIDER.crew.items,
        },
      },
      hospitalityRider: {
        dressingRoom: {
          title: "Dressing Room",
          items:
            generated.hospitalityRider?.dressingRoom ||
            DEFAULT_HOSPITALITY_RIDER.dressingRoom.items,
        },
        catering: {
          title: "Catering",
          items:
            generated.hospitalityRider?.catering ||
            DEFAULT_HOSPITALITY_RIDER.catering.items,
        },
        beverages: {
          title: "Beverages",
          items:
            generated.hospitalityRider?.beverages ||
            DEFAULT_HOSPITALITY_RIDER.beverages.items,
        },
        transport: {
          title: "Transportation",
          items:
            generated.hospitalityRider?.transport ||
            DEFAULT_HOSPITALITY_RIDER.transport.items,
        },
        hotel: {
          title: "Hotel",
          items:
            generated.hospitalityRider?.hotel ||
            DEFAULT_HOSPITALITY_RIDER.hotel.items,
        },
        security: {
          title: "Security",
          items:
            generated.hospitalityRider?.security ||
            DEFAULT_HOSPITALITY_RIDER.security.items,
        },
        guestList: {
          title: "Guest List",
          items:
            generated.hospitalityRider?.guestList ||
            DEFAULT_HOSPITALITY_RIDER.guestList.items,
        },
        payment: {
          title: "Payment",
          items:
            generated.hospitalityRider?.payment ||
            DEFAULT_HOSPITALITY_RIDER.payment.items,
        },
      },
      socials: {
        farcaster:
          generated.farcasterUsername || profile?.username || undefined,
      },
      onChain: {},
    };
