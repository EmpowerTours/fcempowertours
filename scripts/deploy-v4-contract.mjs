/**
 * Deploy EmpowerToursYieldStrategyV4 contract
 *
 * V4 CHANGES:
 * - Renamed stakeWithNFT to stakeWithDeposit (clearer naming)
 * - Eliminates approve + spend pattern for AA compatibility
 * - Safe approves YieldStrategy ONCE with max allowance
 * - Then can stake unlimited times with single transaction
 * - No more batched approve + stake transactions
 */
...
async function compileContract() {
  console.log('📦 Reading V4 contract source...');
  const contractPath = './contracts/EmpowerToursYieldStrategyV4.sol';
  const source = fs.readFileSync(contractPath, 'utf8');

  console.log('🔧 Compiling V4 contract...');

  // Solidity compiler input
  const input = {
    language: 'Solidity',
    sources: {
      'EmpowerToursYieldStrategyV4.sol': {
        content: source,
      },
    },
    settings: {
  ...
  const contract = output.contracts['EmpowerToursYieldStrategyV4.sol']['EmpowerToursYieldStrategyV4'];
...
async function deployContract() {
  try {
    ...
    console.log('');
    console.log('🚀 Deploying EmpowerToursYieldStrategyV4...');
    ...
    if (receipt.status === 'success') {
      ...
      console.log('   Change from V3 to V4 address');
      console.log('2. Update stakeWithNFT to stakeWithDeposit in route.ts');
      console.log('3. Whitelist Passport NFT: 0x54e935c5f1ec987bb87f36fc046cf13fb393acc8');
      console.log('4. Approve Safe to spend TOURS with max allowance (one-time setup)');
      console.log('');
      ...
      console.log('');
      console.log('To approve Safe for unlimited TOURS:');
      console.log(`node scripts/approve-yield-strategy-max.mjs`);
      console.log('');
      console.log('UPDATE .ENV:');
      console.log(`YIELD_STRATEGY_V4=${receipt.contractAddress}`);
      console.log('');
      console.log('VERIFICATION COMMAND:');
      console.log(`forge verify-contract \\`);
      console.log(`  ${receipt.contractAddress} \\`);
      console.log(`  contracts/EmpowerToursYieldStrategyV4.sol:EmpowerToursYieldStrategyV4 \\`);
      console.log(`  --chain 10143 \\`);
      console.log(`  --constructor-args $(cast abi-encode "constructor(address,address,address,address,address)" ${TOURS_TOKEN} ${KINTSU} ${TOKEN_SWAP} ${DRAGON_ROUTER} ${KEEPER})`);
      console.log('');
    } else {
